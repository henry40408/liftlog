...
- **The no-JS path has its own feature.** `features/workouts/no_js.feature` builds a `javaScriptEnabled: false` context (carrying the logged-in cookies across) and walks the confirmation page the other scenarios never see. Any change to a destructive trigger should keep both paths covered.
