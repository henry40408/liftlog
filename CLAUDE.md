...
The `Origin`/`Host` comparison ignores scheme and port **on purpose**, and that tolerance is why this module is hand-rolled rather than delegating to `tower_http::csrf` (issue #187, evaluated and closed). Fetch metadata only reaches potentially-trustworthy origins, so on a plain-HTTP LAN install — a shape the README supports first-class — no `Sec-Fetch-Site` arrives and this comparison is the *only* check running. A byte-exact authority match would 403 every POST, login included, behind `proxy_set_header Host $host;` on a non-default port. Every rejection emits a `csrf.rejected` audit event naming the branch that rejected, since the guard's only other symptom is an unexplained 403.
