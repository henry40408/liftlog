...
**Destructive actions confirm on the server, never in `window.confirm()`.** An `onsubmit="return confirm(…)"` guard does nothing with JavaScript off — the form just posts, and the deed is done unannounced. So every destructive route is registered as `get(confirm_page).post(action)` on one path: the trigger in the list/detail view is an `<a href>`, the GET renders an interstitial via `handlers::confirm::page` (`templates/confirm.html`), and only its POST acts. The GET must be inert and must apply the *same* ownership check as the POST — each one has a test asserting both. `handlers::auth`'s promote/delete-user pages are the same shape plus an admin password re-check, and keep their own template.
...
- **No confirm dialogs — confirmation pages.** Nothing in the app uses `window.confirm()`; steps never need `page.once('dialog', …)`. Every destructive action is a link to a GET interstitial whose POST does the deed, so the step clicks the row action, then the button on the confirmation page (worded in full: `Delete workout`, `Delete set`, `Delete exercise`, `Revoke share link`, `Log out other devices`). Promote-user and delete-user use the same shape but additionally re-check the admin's own password.
